...
const mergepdfs = async (p1, p2) => {
  ...
  let startnum = 1;
  let endnum = 100;
  let randomkeypdf = startnum + (endnum - startnum) * Math.random();
  let printrandomkey = Math.round(randomkeypdf);
  // let uniquename = new Date();
  // uniquename.getMilliseconds();
  await merger.save(`public/${printrandomkey}.pdf`); //save under given name and reset the internal document
  return printrandomkey;
